...
def initialize_three_layer_conv_part2(dtype=torch.float, device='cpu'):
  ...
  fc_w=nn.init.kaiming_normal_(torch.empty(num_classes,channel_2*H*W,device=device,dtype=dtype))
...
class ResidualBlock(nn.Module):
  def __init__(self, Cin, Cout, downsample=False):
    ...
    cut1=nn.Identity() if Cin==Cout else nn.Conv2d(Cin,Cout,kernel_size=1)
    cut2=nn.Identity() if not downsample else nn.Conv2d(Cout,Cout,kernel_size=1,stride=2)
    self.shortcut=nn.Sequential(cut1,cut2)
    ############################################################################
    #                                 END OF YOUR CODE                         #
    ############################################################################
...
class ResNet(nn.Module):
  def __init__(self, stage_args, Cin=3, block=ResidualBlock, num_classes=10):
    ...
    # Replace "pass" statement with your code
    temp=[]
    temp.append(ResNetStem(Cin,stage_args[0][0]))
    for i in stage_args:
      temp.append(ResNetStage(*i,block))
