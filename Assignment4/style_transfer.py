...
def content_loss(content_weight, content_current, content_original):
    ...
    a,b,c,d=content_current.shape
    transfer1=content_current.view(b,c*d)
    transfer2=content_original.view(b,c*d)
    loss=content_weight*((transfer1-transfer2)**2).sum()
    return loss
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
...
def gram_matrix(features, normalize=True):
    ...
    gram=torch.bmm(feat,feat.permute(0,2,1))
...
def style_loss(feats, style_layers, style_targets, style_weights):
    ...
    ##############################################################################
    # Replace "pass" statement with your code
    loss=torch.zeros(1,dtype=feats[0].dtype,device=feats[0].device)
    for i,layer in enumerate(style_layers):
      Gram_current=gram_matrix(feats[layer])
      loss=loss+style_weights[i]*torch.sum((Gram_current-style_targets[i])**2)
    return loss
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
...
def tv_loss(img, tv_weight):
    ...
    x=img[:,:,1:,:]
    y=img[:,:,:,1:]
    loss=tv_weight*(((img[:,:,:H-1,:]-x)**2).sum()+((img[:,:,:,:W-1]-y)**2).sum())
    return loss
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################