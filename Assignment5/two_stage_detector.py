...
class ProposalModule(nn.Module):
  def __init__(self, in_dim, hidden_dim=256, num_anchors=9, drop_ratio=0.3):
    ...
    self.pred_layer=nn.Sequential(nn.Conv2d(in_dim,hidden_dim,kernel_size=(3,3),padding=(1,1)),
                                  nn.Dropout(p=drop_ratio),
                                  nn.LeakyReLU(),
                                  nn.Conv2d(hidden_dim,num_anchors*6,kernel_size=(1,1)))
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
  ...
  def forward(self, features, pos_anchor_coord=None, \
    ...
    A=self.num_anchors
    out=self.pred_layer(features).view(B,A,6,H,W)
    conf_scores=out[:,:,:2,:,:]
    offsets=out[:,:,2:,:,:]
    if mode=='train':
      M=pos_anchor_idx.shape
      pos_conf_scores=self._extract_anchor_data(conf_scores[:,:,:2,:,:],pos_anchor_idx)
      neg_conf_scores=self._extract_anchor_data(conf_scores[:,:,:2,:,:],neg_anchor_idx)
      conf_scores=torch.cat((pos_conf_scores,neg_conf_scores),0)
      offsets=self._extract_anchor_data(offsets,pos_anchor_idx)
      pos_anchor_coord=pos_anchor_coord.unsqueeze(1).unsqueeze(1).unsqueeze(1)
      proposals=GenerateProposal(pos_anchor_coord,offsets.unsqueeze(1).unsqueeze(1).unsqueeze(1),method='FasterRCNN')
...
class RPN(nn.Module):
  ...
  def forward(self, images, bboxes, output_mode='loss'):
    ...
    B,_,H,W=features.shape
    grid=GenerateGrid(B,W,H,features.dtype,features.device)
    anchor=GenerateAnchor(self.anchor_list,grid)
    iou=IoU(anchor,bboxes)
    pos_anchor_idx,neg_anchor_idx,GT_conf_scores,GT_offsets,GT_class,pos_anchor_coord,_=ReferenceOnActivatedAnchors(anchor,bboxes,grid,iou)
    ...
    total_loss=w_conf*conf_loss+w_reg*reg_loss
    anc_per_img=len(self.anchor_list)*H*W
  ...
  def inference(self, images, thresh=0.5, nms_thresh=0.7, mode='RPN'):
    ...
    with torch.no_grad():
      ...
      grid=GenerateGrid(B,H,W,dtype=features.dtype,device=features.device)
      anchor=GenerateAnchor(self.anchor_list,grid)
      conf_scores,offsets=self.prop_module(features) #(B,A,2,H,W) (B,A,4,H,W)
      conf_scores=torch.sigmoid(conf_scores.permute(0,1,3,4,2).contiguous()) #(B,A,H,W,2)
      offsets=offsets.permute(0,1,3,4,2).contiguous() #(B,A,H,W,4)
      proposals=GenerateProposal(anchor,offsets,method='FasterRCNN') #(B,A,H,W,4)
    for idx in range(B):
      mask=(conf_scores[idx,:,:,:,0]>thresh) #(A,H,W)
      conf_score=conf_scores[idx,:,:,:,0][mask] #(N)
      proposal=proposals[idx][mask] #(N,4)
      keep_idx=torchvision.ops.nms(proposal,conf_score,nms_thresh)
      final_conf_probs.append(conf_score[keep_idx].unsqueeze(1))
      final_proposals.append(proposal[keep_idx])
...
class TwoStageDetector(nn.Module):
  ...
  def forward(self, images, bboxes):
    ...
    ##############################################################################
    # Replace "pass" statement with your code
    rpn_loss,conf_scores,proposals,features,GT_class,pos_anc_idx,anc_per_img=self.rpn(images,bboxes,output_mode='all')
    boxes=torch.zeros((proposals.shape[0],5),dtype=proposals.dtype,device=proposals.device)
    boxes[:,0]=(pos_anc_idx//anc_per_img)
    boxes[:,1:]=proposals
    roi=torchvision.ops.roi_align(features,boxes.data,output_size=(self.roi_output_h,self.roi_output_w))
    mean_roi=torch.mean(roi,dim=(2,3))
    class_score=self.cls_layer(mean_roi)
    cls_loss=nn.functional.cross_entropy(class_score,GT_class)
    total_loss=rpn_loss+cls_loss
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
  ...
  def inference(self, images, thresh=0.5, nms_thresh=0.7):
    ...
    with torch.no_grad():
      ...
      roi=torchvision.ops.roi_align(features,final_proposals,(self.roi_output_h,self.roi_output_w)) #(K,_,2,2)
      roi_pool=torch.mean(roi,dim=(2,3)) #(K,_)
      class_score=self.cls_layer(roi_pool) #(K,C)
      label=torch.argmax(class_score,dim=1,keepdim=True)
      idx=[]
      for proposal in final_proposals:
        idx.append(proposal.shape[0])
      final_class=torch.split(label,idx)
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
