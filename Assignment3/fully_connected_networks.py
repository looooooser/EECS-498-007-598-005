...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None):
    ...
    # Replace "pass" statement with your code
    N=X.shape[0]
    out1,cache1=Linear_ReLU.forward(X.view(N,-1),self.params['W1'],self.params['b1'])
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    loss1,dx_upstream=softmax_loss(scores,y)
    loss=loss1+self.reg*(pow(self.params['W1'],2).sum()+pow(self.params['W2'],2).sum())
    dx2,dw2,db2=Linear.backward(dx_upstream,cache2)
...
class FullyConnectedNet(object):
  ...
  def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
    ...
    for i,size in enumerate(hidden_dims,1):
      self.params['W%d'%i]=weight_scale*torch.randn([j,size],dtype=dtype,device=device)
      self.params['b%d'%i]=torch.zeros(size,dtype=dtype,device=device)
      j=size
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
  ...
  def loss(self, X, y=None):
    ...
    for i in range(self.num_layers):
      ...
      if i!=self.num_layers-1:
        X,_=Linear_ReLU.forward(X,w,b)
        if self.use_dropout:
          X,cc=Dropout.forward(X,self.dropout_param)
          dropout_cache.append(cc)
      else:
        X,_=Linear.forward(X,w,b)
      cache.append(_)
    scores=X
    ...
    for i in range(self.num_layers):
      loss+=self.reg*(self.params['W'+str(self.num_layers-i)]**2).sum()
      if i==0:
        dx_upstream,grads['W'+str(self.num_layers-i)],grads['b'+str(self.num_layers-i)]=Linear.backward(dx_upstream,cache[self.num_layers-i-1])
      else:
        if self.use_dropout:
          dx_upstream=Dropout.backward(dx_upstream,dropout_cache[self.num_layers-i-1])
        dx_upstream,grads['W'+str(self.num_layers-i)],grads['b'+str(self.num_layers-i)]=Linear_ReLU.backward(dx_upstream,cache[self.num_layers-i-1])
      grads['W'+str(self.num_layers-i)]+=2*self.reg*self.params['W'+str(self.num_layers-i)]
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
...
def create_solver_instance(data_dict, dtype, device):
  ...
  solver = None
  # Replace "pass" statement with your code
  solver=Solver(model,data_dict,num_epochs=20,optim_config={'learning_rate':0.1},lr_decay=0.95,device=device)
...
class Dropout(object):
  ...
  def forward(x, dropout_param):
    ...
    if mode == 'train':
      ...
      ###########################################################################
      # Replace "pass" statement with your code
      mask=torch.rand(x.shape,device='cuda')<(1-p)
      out=mask*x/(1-p)
      ###########################################################################
      #                             END OF YOUR CODE                            #
      ###########################################################################
