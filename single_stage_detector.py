...
def GenerateAnchor(anc, grid):
  ...
  for j in range(A):
    anchors[:,j,:,:,0]=grid[:,:,:,0]-anc[j,0]/2.0
    anchors[:,j,:,:,1]=grid[:,:,:,1]-anc[j,1]/2.0
    anchors[:,j,:,:,2]=grid[:,:,:,0]+anc[j,0]/2.0
    anchors[:,j,:,:,3]=grid[:,:,:,1]+anc[j,1]/2.0
...
def GenerateProposal(anchors, offsets, method='YOLO'):
  ...
  y1=y-h/2.0
  y2=y+h/2.0
  proposals[:,:,:,:,0],proposals[:,:,:,:,1],proposals[:,:,:,:,2],proposals[:,:,:,:,3]=x1,y1,x2,y2
  ##############################################################################
  #                               END OF YOUR CODE                             #
  ##############################################################################
...
def IoU(proposals, bboxes):
  ...
  x3,y3,x4,y4=label[:,:,:,0],label[:,:,:,1],label[:,:,:,2],label[:,:,:,3]
  left=torch.max(x1,x3)
  right=torch.min(x2,x4)
  intersection=(right-left)*(right>left)
  up=torch.max(y1,y3)
  down=torch.min(y2,y4)
  intersection=intersection*(down-up)*(down>up)
...
class PredictionNetwork(nn.Module):
  def __init__(self, in_dim, hidden_dim=128, num_anchors=9, num_classes=20, drop_ratio=0.3):
    ...
    # Replace "pass" statement with your code
    self.pred_layer=nn.Sequential(nn.Conv2d(in_dim,hidden_dim,kernel_size=(1,1)),
                                  nn.Dropout(drop_ratio),
                                  nn.LeakyReLU(),
                                  nn.Conv2d(hidden_dim,5*num_anchors+num_classes,kernel_size=(1,1)))
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
  ...
  def forward(self, features, pos_anchor_idx=None, neg_anchor_idx=None):
    ...
    data=out[:,:5*A,:,:].view(B,A,5,H,W)
    conf_scores=data[:,:,:1,:,:]
    conf_scores=torch.sigmoid(conf_scores)
    offsets=data[:,:,1:,:,:]
    offsets[:,:,:2,:,:]=torch.sigmoid(offsets[:,:,:2,:,:])
    offsets[:,:,:2,:,:]=offsets[:,:,:2,:,:]-0.5
    class_scores=out[:,5*A:5*A+C,:,:]
    if pos_anchor_idx is not None:
      ...
      temp=conf_scores.clone()
      conf_scores_pos=self._extract_anchor_data(temp,pos_anchor_idx)
      conf_scores_neg=self._extract_anchor_data(temp,neg_anchor_idx)
      conf_scores=torch.cat((conf_scores_pos,conf_scores_neg),0)
    else:
      conf_scores=conf_scores.squeeze(2)
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
...
class SingleStageDetector(nn.Module):
  ...
  def forward(self, images, bboxes):
    ...
    grid=GenerateGrid(feat.shape[0],feat.shape[2],feat.shape[3],dtype=feat.dtype,device=feat.device)
    anchors=GenerateAnchor(self.anchor_list,grid)
    iou=IoU(anchors,bboxes)
    activated_anc_ind,negative_anc_ind,GT_conf_scores,GT_offsets,GT_class,_,_=ReferenceOnActivatedAnchors(anchors,bboxes,grid,iou,neg_thresh=0.2,method='YOLO')
    conf_scores,offsets,class_scores=self.pred_network(feat,activated_anc_ind,negative_anc_ind)
    conf_loss=ConfScoreRegression(conf_scores,GT_conf_scores)
    offset_loss=BboxRegression(offsets,GT_offsets)
    class_loss=ObjectClassification(class_scores,GT_class,feat.shape[0],self.anchor_list.shape[0]*feat.shape[2]*feat.shape[3],activated_anc_ind)
    total_loss=w_conf*conf_loss+w_reg*offset_loss+w_cls*class_loss
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
  ...
  def inference(self, images, thresh=0.5, nms_thresh=0.7):
    ...
    anchors=GenerateAnchor(self.anchor_list,grid)
    with torch.no_grad():
      conf_scores,offsets,class_scores=self.pred_network(feat)   #(B,A,H,W) (B,A,4,H,W) (B,C,H,W)
    proposals=GenerateProposal(anchors,offsets.permute(0,1,3,4,2)) #(B,A,H,W,4)
    for idx in range(feat.shape[0]):
      mask=(conf_scores[idx]>thresh) #(A,H,W)
      max_score=class_scores[idx].max(dim=0)[1] #(H,W)
      class_score=max_score.expand(mask.shape)[mask] #(N)
      conf_score=conf_scores[idx][mask] #(N)
      proposal=proposals[idx][mask]   #(N,4)
      keep_index=torchvision.ops.nms(proposal,conf_score,nms_thresh)
      final_proposals.append(proposal[keep_index])
      final_conf_scores.append(conf_score[keep_index].unsqueeze(1))
      final_class.append(class_score[keep_index].unsqueeze(1))      
...
def nms(boxes, scores, iou_threshold=0.5, topk=None):
  ...
  while index.size()[0]>0:
    ...
    up=torch.max(y1,y3)
    down=torch.min(y2,y4)
    intersection=(right-left)*(right>left)*(down-up)*(down>up)
    union=(x2-x1)*(y2-y1)+(x4-x3)*(y4-y3)-intersection
